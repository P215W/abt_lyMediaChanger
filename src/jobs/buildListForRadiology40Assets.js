...
const result = require("dotenv").config({
  ...
  const IS_RE_RUN_FROM_LAST_SUCCESS_AT = "799"; // default value: null; In case of errors, this should equal the last successfully saved asset id (as string type!, e.g. "11174")
  const RUN_NUMBER = "19"; // default value: "1"; In case of errors, this should be ascending for each new run, i.e. first rerun would be "2" and so forth
  ...
  const SPREADSHEET_NAME = "40% Radiology list";
  const SPREADSHEET_WORKSHEET_NAME = "40% Radiology list"; // for the "Reiter" of a spreadsheet
  
  const LYMEDIA_SEARCH_TITLE = null;
  const LYMEDIA_SEARCH_DESCRIPTION = null;
  const LYMEDIA_SEARCH_AUTHOR = null;
  const LYMEDIA_SEARCH_STATUS = 40; // to select 40%
