...
const result = require("dotenv").config({
  ...
  const SPREADSHEET_NAME = "1er Assets EN";
  const SPREADSHEET_WORKSHEET_NAME = "1er Assets EN"; // for the "Reiter" of a spreadsheet
  
  const LYMEDIA_SEARCH_TITLE = null;
  const LYMEDIA_SEARCH_DESCRIPTION = null;
  const LYMEDIA_SEARCH_AUTHOR = null;
  const LYMEDIA_SEARCH_STATUS = 1; // e.g. number type 40 to select 40%
