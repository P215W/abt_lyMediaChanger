...
async function createMarcsPathoList(imageTypeForSearch, tissueTypeForSearch) {
  ...
  const SPREADSHEET_NAME = "Marcs Patho List";
  const SPREADSHEET_WORKSHEET_NAME = "Marcs Patho List"; // for the "Reiter" of a spreadsheet
  ...
  const LYMEDIA_SEARCH_NEXT_EDITOR = "93653"; // try first string type, then number type
  const LYMEDIA_SEARCH_EXTERNAL_ADDITION_TYPE = null;
  const LYMEDIA_SEARCH_CHOOSECOPYRIGHTS = null;
  // works:
  // const LYMEDIA_SEARCH_REQUIRE_TAGS = "bild:bildtyp=Mikroskopie";
  const LYMEDIA_SEARCH_REQUIRE_TAGS = imageTypeForSearch;
  const LYMEDIA_SEARCH_REQUIRE_TAGS_FOR_TISSUE = tissueTypeForSearch;
  ...
  const dataForTable = originalArray.map((item) => {
    return {
      ...
      isChecked: false,
      title: null,
      currentAssetExternalUrl: null,
      openedPageStatus: null,
      openedPageUrl: null,
    };
  });
...
exports.data = createMarcsPathoList;
