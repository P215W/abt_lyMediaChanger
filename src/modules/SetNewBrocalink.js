async function SetNewBrocalink(
  ...
  await modalPage.waitFor(2000); // gives page some time to find search results for the above title input
  await modalPage.click("#select2-multi_link_new_idForeign-results li"); // chooses first upcoming result (which is expected to be the correct one, since we type the precise original title)
